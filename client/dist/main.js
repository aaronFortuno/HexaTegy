...
function showLobby() {
    ...
    localeUnsub?.();
    rootEl.innerHTML = "";
    new LobbyView(rootEl, RELAY_URL, (relay, roomCode, clientId, isAdmin) => {
...
function showWaitingRoom(relay, roomCode, clientId, isAdmin) {
    rootEl.innerHTML = `
  ...
  `;
    ...
    function renderPlayerList(players) {
        ...
        listEl.innerHTML = players
          ...
          <div class="player-item" data-id="${p.id}">
            ...
            .join("");
        // Kick
        listEl.querySelectorAll(".btn-kick[data-id]").forEach((btn) => {
            btn.addEventListener("click", () => adminHost?.kickPlayer(btn.dataset.id));
        });
        // Editar nom (inline)
        listEl.querySelectorAll(".btn-edit-name[data-id]").forEach((btn) => {
            btn.addEventListener("click", () => {
                const id = btn.dataset.id;
                const item = listEl.querySelector(`.player-item[data-id="${id}"]`);
                if (!item)
                    return;
                const nameSpan = item.querySelector(".player-name");
                if (!nameSpan || item.querySelector(".player-name-input"))
                    return; // ja editant
    ...
    relay.on(MsgType.GAME_STATE, (msg) => {
        ...
        if (state.phase === "lobby") {
            ...
            if (state.config)
                renderConfigPreview(state.config);
 ...
 */
function applyVisibilityFilter(regions, myPlayerId, config) {
    const mode = config?.visibilityMode ?? "full";
    if (mode === "full")
        return regions;
    const myRegionIds = new Set(regions.filter((r) => r.ownerId === myPlayerId).map((r) => r.id));
    ...
    if (mode === "fog_of_war") {
        ...
        return regions.map((r) => {
            if (myRegionIds.has(r.id) || adjacentIds.has(r.id))
                return r;
            return { ...r, troops: 0 };
        });
    }
...
function showGame(relay, clientId, isAdmin, adminHost) {
    rootEl.innerHTML = `
  ...
  `;
    ...
    // ─── Input ───────────────────────────────────────────────────────────────
    const inputHandler = new InputHandler(canvas, hexRenderer, camera, clientId, (orders) => {
        localOrders = orders;
    ...
    inputHandler.onContextMenu((region, cx, cy) => {
    ...
    function showRegionContextMenu(region, clientX, clientY) {
    ...
    function showArrowTroopInput(fromId, toId, screenPos) {
    ...
    function renderFrame() {
        ...
        const myPlayer = game.playerById(clientId);
        const plannedArrows = localOrders.map((o) => ({
...
function showEndScreen(relay, winnerId, game, adminHost) {
    const endScreen = new EndScreen(rootEl);
    endScreen.show(winnerId, game.players, game.regions, () => showWaitingRoom(relay, relay.roomCode ?? "", relay.clientId ?? "", adminHost !== null), () => {
        relay.disconnect();
        adminHost?.destroy();
        showLobby();
    });
