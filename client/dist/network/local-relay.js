...
export class LocalRelay {
    ...
    createRoom() {
        ...
        this.toAdminCh.onmessage = (e) => this.handleIncomingAsAdmin(e.data);
    ...
    joinRoom(roomCode, name) {
        ...
        this.fromAdminCh.onmessage = (e) => this.dispatch(e.data);
        // Canal privat per rebre la confirmació de l'admin (room:joined)
        const privateCh = new BroadcastChannel(`hexategy:${roomCode}:to-${this.clientId}`);
        privateCh.onmessage = (e) => {
    ...
    handleIncomingAsAdmin(msg) {
        // Cas especial: protocol d'unió a sala
        if (msg.type === MsgType.ROOM_JOIN) {
            const { name, clientId } = msg.payload;
            // Confirmar al jugador específic via canal privat
            const playerCh = new BroadcastChannel(`hexategy:${this.roomCode}:to-${clientId}`);
