 ...
 */
import { MsgType, DEFAULT_CONFIG } from "./protocol.js";
...
export class AdminHost {
    ...
    static RESOLVE_GRACE_MS = 1000;
    startRound(skipProductionFor = new Set()) {
        this.round++;
        this.phase = "planning";
        ...
        this.roundTimer = setTimeout(() => this.resolveRound(), this.config.roundDuration * 1000 + AdminHost.RESOLVE_GRACE_MS);
    }
    resolveRound() {
        if (this.phase !== "planning")
            return;
        this.phase = "resolving";
        const allOrders = [...this.pendingOrders.entries()].flatMap(([playerId, orders]) => orders.map((o) => ({ ...o, playerId })));
        ...
        const result = resolveRound(this.regions, allOrders, this.config);
        ...
        const newlyConquered = new Set(this.regions
        ...
        this.relay.send(MsgType.ROUND_RESOLVE, result);
        // Comprovar victòria
    ...
    broadcastState() {
        ...
        this.relay.send(MsgType.GAME_STATE, payload);
    }
